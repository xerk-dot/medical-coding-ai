...
2. **Fallback Parsing** (lines 114-131): If tool calls fail, the system uses several fallback parsers:
    ...
    - `_parse_json_response()`: For models that return JSON instead of tool calls

3. **Text Pattern Matching** (lines 169-229): The `_parse_response()` method looks for patterns such as:
    - `A:`, `B.`, `C)` at the start of a line
    - `"Answer: A"` patterns
